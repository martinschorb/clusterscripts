...
parser.add_argument('-m', dest='mont', help='use this flag if it is a montaged tomogram', action='store_true',
                    default=False)
parser.add_argument('-d', dest='dual', help='use this flag if it is a dual-axis tomogram', action='store_true',
                    default=False)
parser.add_argument('-b', dest='binning', help='set the binning to be used for image analysis, default is 4', type=int,
                    default=4)
...
callcmd = 'batchruntomo -root \"' + base1 + '\" -directive \"' + dir_file + '\" -current . -start 4 -cp ' + str(
    cpus) + ' -em ' + user + '@embl.de'
os.system(callcmd)
