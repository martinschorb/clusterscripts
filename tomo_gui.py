...
def cluster_tomo(*args):
    ...
    with open('tomo_submit.sh', 'w') as file:
        file.writelines('\n'.join(commands))
...
def browse_adoc(*args):
    cdir = currdir.get()
    adoc_in = filedialog.askopenfilename(parent=root, title='Choose batch directive file', initialdir=cdir,
                                         filetypes=(('Batch directives', '*.adoc'), ('all files', '*.*')))
    adoc.set(adoc_in)
...
def jobs(*args):
    ...
    p = subprocess.Popen('squeue -u `whoami` -o "%.8i %.9P %.8j %.8u %.2t %.10M %.6C %.6D %16R %30o"',
